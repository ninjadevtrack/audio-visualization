...
const ScaleBar = {
  ...
  get: function (fromMin, fromMax, valueIn) {
    ...
    return (
      ((toMax - toMin) * (valueIn - fromMin)) / (fromMax - fromMin) + toMin
    );
  },
};
...
function genWav(json, ap) {
  ...
  document.querySelector("#song").innerHTML = json.name;
  document.querySelector("#cover").src = json.album.images[0].url;
  document.querySelector(".vis-circle").style.backgroundImage =
    "url(" + json.album.images[0].url + ")";
  ...
  jsmediatags.read(file, {
    onSuccess: ({ tags }) => {
      document.querySelector("#artist").innerHTML =
        tags.artist === undefined ? fileInfo.artist : tags.artist;
      document.querySelector("#album").innerHTML =
        tags.album === undefined ? "" : tags.album;
      document.querySelector("#song").innerHTML =
        tags.title === undefined ? fileInfo.title : tags.title;
...
document.querySelector("#show-audio").onclick = () =>
